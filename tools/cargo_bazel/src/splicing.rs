...
type ExtraManifestsDigest = Vec<ExtraManifestInfo>;
...
pub struct SplicingManifest {
    ...
    pub extra_manifest_infos: ExtraManifestsDigest,
...
impl<'a> SplicerKind<'a> {
    ...
    pub fn splice(&self, workspace_dir: &Path, generator: &LockGenerator) -> Result<PathBuf> {
        let get_extra_workspace_manifests =
            |extra_manifests: &ExtraManifestsDigest| -> Result<HashMap<PathBuf, Manifest>> {
                extra_manifests
                    .iter()
        ...
        match self {
            SplicerKind::Workspace {
                ...
                let extra_workspace_manifests =
                    get_extra_workspace_manifests(&splicing_manifest.extra_manifest_infos)?;
...
mod test {
    ...
    fn mock_extra_manifest_digest(cache_dir: &Path) -> ExtraManifestsDigest {
        vec![{
